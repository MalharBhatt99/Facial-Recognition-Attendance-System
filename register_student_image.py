...
def recognize_student_image(roll,name,image):
        image_path = os.path.join(DATASET_DIR, f"{roll}_{name}.jpg")
        with open(image_path, "wb") as f:
            f.write(image.read())
        st.success("Student's Image is Successful (upload mode)")
        return image_path
...
def register_student_image_gui(roll_no,name,semester,branch,section):
    st.subheader("📝 Register a New Student")

    filename = f"{roll_no}_{name.replace(' ', '_')}.jpg"

    ##image_capturing starts here
    image_path = recognize_student_image(roll_no,name,filename)
